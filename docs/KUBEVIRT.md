...
The first VM imports the Ubuntu 24.04 cloud image through CDI, stores it
in a `local-path` PVC, injects cloud-init user data, and attaches to the
pod network with masquerade. Lab-only NodePort Services expose SSH and
HTTP through the outer Ubuntu node. The guest `devops` account is
key-only and does not have sudo privileges.
